...
class AZtutorial:
    ...
    def create_memb_top(self, fname, lig_itps, mols, topol, sys_name):
        ...
        with open(fname, 'w') as fout:
            for line in lines:
                line = line.replace("toppar", f"{simDir}/{self.membranePath}/toppar")
                line = line.replace("Title", sys_name)
                if 'forcefield.itp' in line:
                    fout.write(line)
                    for i in lig_itps:
    ...
    def assemble_systems_old( self, edges=None ):
        """
        Assembles the systems by combining hybrid structure and protein. Generates input .pdb and .top
        for both the protein+mergedLigs and mergedLigs only
        """
        print('----------------------')
        print('Assembling the systems')
        print('----------------------')
        ...
        for edge in edges:
            ...
            self._create_top(edge,fname=protTopFname,itp=itps,mols=mols,systemName=systemName) ##BertaChanged          
        print('DONE')            
    ...
    def prepare_simulation_old( self, edges=None, simType='em', bLig=True, bProt=True, extra_flag=None):
        """
        Prepare tpr files for simulation. 
        """
            ...
            for state in self.states:
                for r in range(1,self.replicas+1):
                    ...
                    if bLig==True:
                        ...
                        empath = self._get_specific_path(edge=edge,wp=wp,state=state,r=r,sim='em')
                        toppath = ligTopPath
                        self._prepare_single_tpr( simpath, toppath, state, simType, empath, nvtpath,extra_flag=extra_flag)
    ...
    def prepare_jobscripts( self, edges=None, simType='em', bLig=True, bProt=True, bMemb=True, neql=False ):
                ...
                for r in range(1,self.replicas+1):            
                    ...
                    if bLig==True:
                        ...
                        if simType=='transitions':
                            if neql is True:
                                self._commands_for_neql_transitions( simpath, job )
                            else:
                        ...
                        # Remove SBATCH -t 
                        if self.JOBpartition != '':
                            self._rm_SLURM_time(jobfile=jobfile)
                    ...
                    if bProt==True:
                        wp = 'protein'
                        self.jobscripts_membrane(wp, edge, jobfolder, state, r, counter, simType, neql)
                        counter += 1
                    # membrane
                    if bMemb==True:
                        wp = 'membrane'
    ...
    def jobscripts_membrane( self, wp, edge, jobfolder, state, r, counter, simType='em', neql=False ):
        simpath = self._get_specific_path(edge=edge,wp=wp,state=state,r=r,sim=simType)
        jobfile = '{0}/jobscript{1}'.format(jobfolder,counter)
        ...
        if simType == 'em':
            ...
            self._clean_backup_files(simpath) ##BertaChanged: clean backup files, just in case                        
        job.create_jobscript()
        # Remove SBATCH -t, for clusteriqac 
        if self.JOBpartition != '':
            self._rm_SLURM_time(jobfile=jobfile)

    def prepare_jobscripts_old( self, edges=None, simType='em', bLig=True, bProt=True,neql=False ):
        """
        Prepare jobscripts for simulation on cluster nodes
        ##BertaChanged neql = parameter to indicate we will run transition with non equal lambda steps. ONLY FOR TRANSITIONS 
        """
        print('---------------------------------------------')
        print('Preparing jobscripts for: {0}'.format(simType))
        print('---------------------------------------------')
    ...
    def _rm_SLURM_time( self, jobfile): ##BertaChanged
        """
        Removes line containing max job time for job
        """
        with open(jobfile, 'r') as f:
            lines = f.readlines()
        lines = [line for line in lines if not line.startswith('#SBATCH -t')]
    ...
    def _commands_for_neql_transitions( self, simpath, job ):
        """
        Define commands for scripts for non equal lambda transitions simulations
        """

        lambda2, lambda3, top = self._get_info_from_path(simpath)

        for i in range(1,self.frameNum+1):
            if self.JOBqueue=='SGE':
                print("ERROR: This functionality not yet implemented")
            #     cmd0 = f'export GMX_MAXBACKUP={self.frameNum + 10}' ##BertaChanged add 10 just in case
            #     cmd1 = 'cd $TMPDIR'
            #     cmd2 = 'cp {0}/ti$SGE_TASK_ID.tpr tpr.tpr'.format(simpath)
            #     cmd3 = '$GMXRUN -s tpr.tpr -dhdl dhdl$SGE_TASK_ID.xvg'.format(simpath)
            ...
            #     job.cmds = [cmd0,cmd1,cmd2,cmd3,cmd4]
                ...
                cmd1 = 'cd {0}'.format(simpath)
                cmd2 = f'for i in {{1..{self.frameNum}}};do' ##BertaChanged
                cmd3 = '$GMXRUN -s ti$i.tpr -dhdl wlist$i-1 -c str$i-1.gro'
                
                cmd4 = f"gmx grompp -f {lambda2} -c str$i-1.gro -p {top} -o ti$i-2.tpr -maxwarn 4 "
                cmd5 = '$GMXRUN -s ti$i-2.tpr -dhdl wlist$i-2 -c str$i-2.gro'
                
                cmd6 = f"gmx grompp -f {lambda3} -c str$i-2.gro -p {top} -o ti$i-3.tpr -maxwarn 4"
                cmd7 = '$GMXRUN -s ti$i-3.tpr -dhdl wlist$i-3 -c str$i-3.gro'
                cmd8 = "python /home/ramon/berta/aztuto/utils/catdhdls.py $i"
                cmd9 = 'done'
                job.cmds = [cmd0,cmd1,cmd2,cmd3,cmd4,cmd5,cmd6,cmd7,cmd8,cmd9]

    def _get_info_from_path(self,directory):
        """
        Extract info from folders in path
        """

        # Get file paths of lambda mdp files
        if "stateA" in directory:
            state = 'l0'
        elif "stateB" in directory:
            state = 'l1'
        
        lambda2 = os.path.join(self.mdpPath, f"ti_lambdas2_{state}.mdp")
        lambda3 = os.path.join(self.mdpPath, f"ti_lambdas3_{state}.mdp")


        # Get topology path
        toppath = os.path.join(directory.split("state")[0],"topol.top")

        return lambda2, lambda3, toppath
    ...
    def _submission_script( self, jobfolder, counter, simType='eq' ):
        ...
        fp = open(fname,'w')
        fp.write('#!/bin/bash\n')
        fp.write(f'#SBATCH --job-name={simType}\n')
        fp.write(f'#SBATCH --output=job_%A_%a.out\n')
        fp.write(f'#SBATCH --partition=long\n')
        fp.write(f'#SBATCH --cpus-per-task=8\n')
    ...
    def _submission_script_old( self, jobfolder, counter, simType='eq' ):
        """
        Submission script
        ...
        print(f"Will run {self.slotsToUse} jobs max at the same time") ##BertaChanged
        fname = '{0}/submit.py'.format(jobfolder)
        fp = open(fname,'w')
        fp.write('import os\n')
        fp.write('for i in range(0,{0}):\n'.format(counter))
        if self.JOBqueue=='SGE':
            cmd = f"f'qsub jobscript{{i}}\'" ##BertaChanged
            if simType=='transitions':
                cmd = f"f'qsub -t 1-{self.frameNum}:1 jobscript{{i}}\'" ##BertaChanged
        elif self.JOBqueue=='SLURM':
            cmd = '\'sbatch jobscript{0}\'.format(i)'
        fp.write(f'    os.system(f"sbatch -J {simType}_{{i%{self.slotsToUse}}} --dependency=singleton jobscript{{i}}")\n') ##BertaChanged
        fp.close()
    ...
    def prepare_transitions_old( self, edges=None, bLig=True, bProt=True, bGenTpr=True,extra_flag_extractTime=None, extra_flag_sim=None ):
        """
        Prepare transitions tprs
        """
        print('---------------------')
        print('Preparing transitions')
        print('---------------------')

        self._prepareExtractionTime(extra_flag=extra_flag_extractTime) ##BertaChanged
