...
Remember to align the protein to the Z axis and use the AMBER FF. The membrane must be parametrised with AMBER for better reliability with GAFF2, with which the small molecules are parametrised.
...
Prepare a bilayer membrane system that is consistent with the protein-membrane system. This means using the same kind of water (TP3, OPC...), the same AMBER FF and the same lipids that are on the membrane (usually only POPC). 
...
Take the *gromacs* folder and place it in your workspace. Rename it as `membrane`. Even though the program will only care about the *gro* file, you can have the *mdp* files in `mdppath` replaced and adapted (if the membrane is not solely POPC) by the ones generated by CHARMM-GUI after a proper readjustment.
...
pname: 'NA'                            # Which positive ions to use in the ligand with water simulations
nname: 'CL'                            # Which negative ions to use in the ligand with water simulations 
...
JOBexport:                             # List of environment variables to export for the simulation jobs 
  - 'SET_SOME_VAR=1'

JOBsource:                             # List of source files to run before the simulation jobs
  - 'file.sh'                          
...
framesAnalysis: [50,199] 			   # List of frames to analyze. If you want to analyze all frames, ignore this field. If you want to analyze a specific range of frames, you can specify it like this: framesAnalysis: [start_frame, end_frame]
units: kJ                              # Units for the results.
```
...
On the other hand, it is important that the GROMACS executable `gmx` exists. If you only have `gmx_mpi` as an executable, the program will fail. If you want to rapidly change the simulation time of the production (md) or the transitions (ti), you can use the `mdtime` and `titime` parameters, respectively (if these parameters are not set, the values of the _mdppath_ files will be used). When running the preparation, the times will automatically update. The program assumes $dt = 0.002$ ps, and `saveFrames` frames will be saved (default is 400).  

Other options for the input file are:

**For the water system**
* _boxshape_: Shape of the box for the water system, default is dodecahedron.
* _boxd_: `d` parameter of gmx solvate. Only for the water system. Default is 1.5.
* _water_: Water type for the water system (default is tip3p)
* _conc_: Concentration of salt for the system. default is 0.15 nM

**Other**
* _bootstrap_: Number of bootstrap samples.
* _JOBqueue_: job manager (default is SLURM)
* _JOBbGPU_: If GPU is used or not, the default is True
* _JOBbackup_: Gromacs backup files for transitions
